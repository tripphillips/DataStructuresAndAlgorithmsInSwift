...
class PrimTests: XCTestCase {
    ...
    func test_produceMinimumSpanningTree() {
        
        let sut = makeSUT()
        
        let (cost, mst) = Prim().produceMinimumSpanningTree(for: sut)
        print(mst)
        XCTAssertEqual(cost, 15.0)
    }
    
    private func makeSUT() -> AdjacencyList<Int> {
