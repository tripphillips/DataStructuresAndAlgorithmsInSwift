...
class DijkstraTests: XCTestCase {
    ...
    func test_shortestPath_Dijkstra() {
        
        let (vertexA, vertexB, dijkstra) = makeSUT()
        let pathsFromA = dijkstra.shortestPath(from: vertexA)
        let path = dijkstra.shortestPath(to: vertexB, paths: pathsFromA)
        let pathSourceValues = path.map { $0.source.data }
        XCTAssertEqual(pathSourceValues, ["A", "G", "C", "E"])
    }

    private func makeSUT() -> (Vertex<String>, Vertex<String>, Dijkstra<String>) {
        ...
        return (a, b, Dijkstra(graph: graph))
    }
